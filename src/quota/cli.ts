  ...
  try {
    switch (command) {
      ...
      case 'use': {
        ...
        if (!user || Number.isNaN(amount)) throw new Error('use requires --user and optional --amount')
      ...
      case 'grant': {
        ...
        if (!actor || !user || Number.isNaN(amount)) throw new Error('grant requires --actor, --user, --amount')
      ...
      case 'set': {
        ...
        if (!actor || !user || Number.isNaN(limit)) throw new Error('set requires --actor, --user, --limit')
      ...
      case 'status': {
        const user = parsed.user
        if (!user) throw new Error('status requires --user')
